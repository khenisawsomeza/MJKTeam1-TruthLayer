...
function getDefaultAnalysis(text, errorMsg) {
    return {
        ...
        keyRiskIndicators: ["Error: " + (errorMsg || "Could not analyze content.")]
    };
}

async function processPost(postElement) {
    if (fbPaused) return;
    // Remove early return for dismissed posts - we need to reach the restore logic below

    const text = postElement.innerText || "";
    const signature = text.length + "_" + text.substring(0, 30).replace(/\s/g, '');
    
    // 1. Check for immediate rehydration from session cache
    if (postElement._truthlayerData) {
        if (!postElement.querySelector('.truthlayer-floating-popup, .truthlayer-restore-icon')) {
            debugLog('Restoring popup from element cache (unpause/scroll).');
            injectBanner(postElement, postElement._truthlayerData);
            return;
        }
        return; // Already has UI
    }

    // 3. Check persistent storage cache for dismissed state (now synchronous)
    if (cachedDismissedPosts[signature]) {
        debugLog(`Restoring dismissed icon from storage cache for signature: ${signature}`);
    ...
    // 4. Skip if already processed in this session but has no UI (e.g. text too short)
    if (postElement.dataset.truthlayerProcessed === "true" && postElement.dataset.truthlayerSignature === signature) {
        return; 
    }

    // Removed mandatory link filtering to allow popups on all text posts.
    debugLog(`Analyzing post text (length: ${text.length})...`);
    let extractedText = extractPostText(postElement);

    if (!extractedText || extractedText.length < 50) {
        debugLog(`Skipping post signature ${signature}: Extracted text too short (${extractedText?.length || 0} chars).`);
        postElement.dataset.truthlayerProcessed = "true";
        postElement.dataset.truthlayerSignature = signature;
        return;
    }
    ...
    try {
        chrome.runtime.sendMessage({
            ...
            if (chrome.runtime.lastError) {
                console.error('[TruthLayer] Runtime error:', chrome.runtime.lastError.message);
                return;
            }
            if (response && response.success) {
                debugLog('Analysis success. Injecting UI.');
        ...
        console.error('[TruthLayer] Communication error:', error);
    }
}
